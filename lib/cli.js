var Cli = module.exports,
    ...
    MyBuddySystemLib = require('../my-buddy-system-lib'),
    ...
    Utils = MyBuddySystemLib.utils,
    logging = MyBuddySystemLib.logging,
    //Q = require('q'),
...
Cli.lookupTask = function lookupTask(module) {
    try {
        var taskModule = require(module).MyBuddyTask;
        return taskModule;
    } catch (ex) {
        throw ex;
    }
...
Cli.fail = function fail(msg, taskHelp) {
    try {
        ...
        console.error('An error occurred in Ionic App Lib and no error handler was set.');
        console.error(msg);
        process.exit(1);
        return '';
    } catch (ex) {
